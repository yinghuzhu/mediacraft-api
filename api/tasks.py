...
def submit_task():
    ...
    try:
        sid = getattr(g, 'sid', None)
        if not sid:
            logger.warning(f"Task submission attempt without valid session from IP: {get_client_ip(request)}")
            return jsonify({'error': 'No valid session'}), 401
        
        logger.info(f"[TASK_SUBMIT] Task submission started - session_id: {sid}, IP: {get_client_ip(request)}")
        ...
        if not task_type:
            logger.warning(f"[TASK_SUBMIT] Missing task_type - session_id: {sid}, IP: {get_client_ip(request)}")
            return jsonify({'error': 'Missing task_type'}), 400
        
        if task_type not in ['watermark_removal', 'video_merge']:
            logger.warning(f"[TASK_SUBMIT] Invalid task_type: {task_type} - session_id: {sid}, IP: {get_client_ip(request)}")
            return jsonify({'error': 'Invalid task_type'}), 400
        
        logger.info(f"[TASK_SUBMIT] Task type: {task_type} - session_id: {sid}, IP: {get_client_ip(request)}")
        ...
        logger.info(f"[TASK_SUBMIT] Task submitted successfully - task_id: {task_id}, task_type: {task_type}, session_id: {sid}, IP: {get_client_ip(request)}")
        ...
        logger.error(f"[TASK_SUBMIT] Task submission failed - session_id: {sid if 'sid' in locals() else 'unknown'}, error: {e}, IP: {get_client_ip(request)}")
        return jsonify({
...
def get_task_status(task_id):
    ...
    try:
        sid = getattr(g, 'sid', None)
        if not sid:
            logger.warning(f"Task status request without valid session - task_id: {task_id}, IP: {get_client_ip(request)}")
            return jsonify({'error': 'No valid session'}), 401
        
        logger.debug(f"[TASK_STATUS] Status request - task_id: {task_id}, session_id: {sid}, IP: {get_client_ip(request)}")
        ...
        if not task:
            logger.warning(f"[TASK_STATUS] Task not found - task_id: {task_id}, session_id: {sid}, IP: {get_client_ip(request)}")
            return jsonify({'error': 'Task not found'}), 404
        
        # 验证任务所有权
        if task.get('sid') != sid:
            logger.warning(f"[TASK_STATUS] Access denied - task_id: {task_id}, task_owner: {task.get('sid')}, requester: {sid}, IP: {get_client_ip(request)}")
            return jsonify({'error': 'Access denied'}), 403
